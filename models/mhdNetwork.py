...
class MhdNetwork(nn.Module):
    '''
    Implements feed-forward network with MH-Dropout and WTA loss.

    Stochastic WTA loss: Change subset ratio 0<r<1
    ...
    def __init__(self, 
        ...
        if use_pre:
            self.pre_layer = MLP(inp_dim, hid_dim, mhd_hid_dim, num_layers=num_layers, act_fn=act_fn)
        ...
        self.final_layer = MLP(mhd_hid_dim, hid_dim, out_dim, num_layers=1, out_act_fn=out_act_fn, act_fn=act_fn)
    ...
    def get_hypotheses(self, x, hypo_count=None, dropout=True):
        ...
        gate_idx = None
        if dropout:
            x, gate_idx = self.mhd_dropout(
                x=x, hypo_count=hypo_count)
        
        hypotheses = self.final_layer(x)
